...
import { Landmark, Building2, Wind, Trees } from "lucide-react";

interface ExpertiseProps {
  theme?: "light" | "dark";
}

export default function Expertise({ theme = "dark" }: ExpertiseProps) {
  ...
  return (
    <section
      ...
      className={`py-24 transition-colors duration-500 relative overflow-hidden ${
        isLight ? "bg-stone-100 border-t border-stone-200" : "bg-charcoal-900 border-t border-white/5"
      }`}
    >
      {/* Structural horizontal blueprint markings */}
      <div className={`absolute top-0 left-0 w-full h-[1px] ${
        isLight ? "bg-gradient-to-r from-transparent via-charcoal-900/10 to-transparent" : "bg-gradient-to-r from-transparent via-white/10 to-transparent"
      }`}></div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="max-w-xl mb-16">
          ...
          <h2 className={`font-serif text-3xl md:text-5xl font-medium tracking-tight transition-colors duration-500 ${
            isLight ? "text-charcoal-900" : "text-white"
          }`}>
            Consulting Disciplines
          </h2>
          <p className={`mt-4 text-sm md:text-base font-sans font-light leading-relaxed transition-colors duration-500 ${
          ...
          {expertises.map((exp, index) => {
            ...
            return (
              <div
                ...
                className={`group relative border rounded-sm p-8 md:p-10 flex flex-col justify-between overflow-hidden transition-all duration-500 min-h-[280px] ${
                  isLight
                    ? "bg-white border-stone-200 shadow-sm hover:border-bronze hover:shadow-md"
                    : "bg-charcoal-950 border-white/5 hover:border-bronze/40"
                }`}
              >
                ...
                <div
                  className={`absolute inset-0 bg-[radial-gradient(#a37c54_1px,transparent_1px)] [background-size:16px_16px] pointer-events-none transition-opacity duration-700 ${
                    isHovered ? (isLight ? "opacity-4" : "opacity-3") : "opacity-0"
                  }`}
                ></div>
                ...
                <div className="space-y-4 relative z-10">
                  <div className="flex items-center justify-between">
                    <div className={`w-12 h-12 rounded-sm border flex items-center justify-center text-bronze group-hover:text-white group-hover:border-bronze transition-colors duration-500 ${
                      isLight ? "bg-stone-50 border-stone-200" : "bg-charcoal-900 border-white/10"
                    }`}>
                      <Icon size={22} />
                    </div>
                    <span className={`text-[9px] font-mono tracking-[0.2em] group-hover:text-bronze transition-colors ${
                      isLight ? "text-charcoal-400" : "text-charcoal-400"
                    }`}>
                      {exp.tag}
                    </span>
                  </div>

                  <h3 className={`font-serif text-2xl font-medium tracking-tight mt-4 group-hover:text-bronze transition-colors ${
                    isLight ? "text-charcoal-900" : "text-white"
                  }`}>
                    {exp.title}
                  </h3>
                  
                  <p className={`text-sm font-sans font-light leading-relaxed transition-colors ${
                    isLight
                      ? "text-charcoal-600 group-hover:text-charcoal-800"
                ...
                <div className={`mt-8 pt-4 border-t flex items-center justify-between relative z-10 ${
                  isLight ? "border-stone-200" : "border-white/5"
                }`}>
                  <span className={`text-[10px] font-mono uppercase tracking-widest group-hover:text-bronze transition-colors ${
                    isLight ? "text-charcoal-400" : "text-charcoal-400"
                  }`}>
                    Active commissions
                  </span>
